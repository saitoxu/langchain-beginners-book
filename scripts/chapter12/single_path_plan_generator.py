...
class SinglePathPlanGenerationState(BaseModel):
    ...
    current_taks_index: int = Field(default=0, description="現在実行中のタスクの番号")
...
class SinglePathPlanGenerator:
    ...
    def _create_graph(self) -> StateGraph:
        ...
        graph.add_conditional_edges(
            "execute_task",
            lambda state: state.current_taks_index < len(state.tasks),
    ...
    def _goal_setting(
        self,
        state: SinglePathPlanGenerationState,
    ) -> dict[str, Any]:
    ...
    def _decompose_query(
        ...
        decomposed_tasks = self.query_decomposer.run(state.optimized_goal)
    ...
    def _execute_task(
        ...
        current_task = state.tasks[state.current_taks_index]
        result = self.task_executor.run(current_task)
        return {
            "results": [result],
            "current_taks_index": state.current_taks_index + 1,
        }
...
class QueryDecomposer:
    ...
    def run(self, query: str) -> DecomposedTasks:
        prompt = ChatPromptTemplate.from_template(
            ...
            "2. 各タスクは具体的かつ詳細に記載されており、単独で実行ならびに検証可能な情報を含めること。"
            "一切抽象的な表現を含まないこと。\n"
...
if __name__ == "__main__":
    generator = SinglePathPlanGenerator()
    query = "猫砂が飛び散らない方法"
